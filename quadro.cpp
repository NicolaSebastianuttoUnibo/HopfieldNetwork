...
///questa funzione prende il path della immagine e la converte in "pixel"
 float threshold = 128.0f ;
    ...
    unsigned char *image_data = stbi_load(Quadro::Quadro::path_.c_str(), &img_width, &img_height, &img_channels, 0);
   if (image_data == nullptr) {
        throw std::runtime_error(("Errore: Impossibile caricare l'immagine: " + Quadro::Quadro::path_).c_str());
    }
 ...
 int luminance = 0;
            ...
            if (pixel_count > 0) {
             ...
             if (average_luminance > threshold) {
                ...
                Quadro::Quadro::pattern_.push_back(0); // Scuro (o -1 se la tua rete di Hopfield usa -1)
            }
}}
