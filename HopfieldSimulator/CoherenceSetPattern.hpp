...
namespace HN {
template <typename, typename> class HopfieldNetwork;
}
namespace CSP {

template <typename T = int8_t> class CoherenceSetPattern {
...
public:
  explicit CoherenceSetPattern(const std::string &path,
                               const std::size_t numColumns,
                               const std::size_t numRows);
  explicit CoherenceSetPattern(const float noise, const std::size_t numColumns,
                               const std::size_t numRows);
  ...
  bool
  hasSameDimensionOf(CoherenceSetPattern &anotherCoherenceSetPattern) const;
  ...
  void resolveEvolvingPattern(HN::HopfieldNetwork<T, MatrixType> &network,
                              std::atomic<float> *status) {

    network.resolvePattern(this->ep_.getPattern(), this->ep_.getEnergy(),
                           status);
  }
  void clearEnergy() { ep_.getEnergy().clear(); }
};
