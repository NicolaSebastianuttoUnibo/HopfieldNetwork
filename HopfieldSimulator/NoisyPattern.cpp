...
void NP::NoisyPattern<T>::generateCorruptedPattern(const float noise) {
  if (noise < 0.0f || noise > 1.0f) {
    throw std::invalid_argument(
        "The noise must be in the interval [0.0 , 1.0]");
  }
...
// constructor
template <typename T>
NP::NoisyPattern<T>::NoisyPattern(const std::vector<T> &sourcePattern,
                                  const float noise)
    : pattern_(sourcePattern) {
...
void NP::NoisyPattern<T>::cyclePixelState(const std::size_t pixelIndex) {
  ...
  pattern_[pixelIndex] =
      POINTS<T>[(std::distance(POINTS<T>.begin(), it) + 1) % POINTS<T>.size()];
}
