...
template <typename T>
void TP::RandomTrainingPattern<T>::generateRandomPattern(
    const float randomDensity, const std::size_t numColumns,
    const std::size_t numRows) {

  if (randomDensity < 0.0f || randomDensity > 1.0f) {
    throw std::invalid_argument(
        "The random density must be in the interval [0.0 , 1.0]");
  }
  const size_t N = numColumns * numRows;
  this->pattern_.clear();
  this->pattern_.reserve(N);
  static std::uniform_real_distribution<float> real(0.0f, 1.0f);
  static std::uniform_int_distribution<int> integer(
      0, static_cast<int>(POINTS<T>.size() - 1));
  ...
  for (size_t i = 0; i < N; i++) {
    if (real(generator) < randomDensity) {
      ...
      this->pattern_.push_back(POINTS<T>[0]);
    }
  }
}
...
TP::RandomTrainingPattern<T>::RandomTrainingPattern(
    const float randomDensity, const std::size_t numColumns,
    const std::size_t numRows)
...
// regrid()
template <typename T>
void TP::RandomTrainingPattern<T>::regrid(const std::size_t numColumns,
                                          const std::size_t numRows) {
