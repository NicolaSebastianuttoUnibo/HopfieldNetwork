...
class HopfieldSimulator {
  ...
  std::vector<std::unique_ptr<CSP::CoherenceSetPattern<neurons_type>>>
      patterns_;
...
public:
  ...
  getPatterns() const noexcept;

  void clear(const int index);
  void corruptPattern(const size_t index, const float noise = 0.1f);
  void emplace_pattern(const std::string &path, const int numColumns,
                       const int numRows);
  void emplace_pattern(const float noise, const int numColumns,
                       const int numRows);
  void cyclePixelStateOnPattern(const size_t index, const size_t pixelPos);
  void generatePattern(const float noise, const std::size_t numColumns,
  ...
  void setTraining(const int numColumns, const int numRows,
