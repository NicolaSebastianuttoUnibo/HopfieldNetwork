...
const matrix_type HN::HopfieldNetwork<neurons_type, matrix_type>::localField(
    const int index, const std::vector<neurons_type> &input) {

  const int num_neurons = input.size();

  if (num_neurons != W_ij.rows() || num_neurons != W_ij.cols()) {
    throw std::invalid_argument(
        "Dimensioni incompatibili tra pattern e matrice dei pesi.");
  }
...
HN::HopfieldNetwork<neurons_type, matrix_type>::getTraining() {
...
template <typename neurons_type, typename matrix_type>
void HN::HopfieldNetwork<neurons_type, matrix_type>::setTraining(
...
// calculateEnergy()
template <typename neurons_type, typename matrix_type>
float HN::HopfieldNetwork<neurons_type, matrix_type>::calculateEnergy(
...
HN::HopfieldNetwork<neurons_type, matrix_type>::resolvePattern(
    const std::vector<neurons_type> &array) const {
  ...
  for (int i = 0; i < num_neurons; ++i) {
    ...
    for (const auto &candidate_point : POINTS<neurons_type>) {
      double dist_sq =
          std::norm(sum - custom_cast<matrix_type>(candidate_point));
...
void HN::HopfieldNetwork<neurons_type, matrix_type>::resolvePattern(
    std::vector<neurons_type> &getVector, std::vector<float> &getEnergy,
  ...
  if (num_neurons != W_ij.rows()) {
    throw std::invalid_argument("Pattern/matrix dimensions not compatible.");
  }
  ...
  for (int i = 0; i < num_neurons; ++i) {
    ...
    float delta = -(std::real(conjugated_diff) * std::real(local_field) -
    ...
    if (status) {
      ...
      status->store(static_cast<float>(i + 1) / num_neurons,
