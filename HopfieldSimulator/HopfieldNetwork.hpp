...
namespace CSP {
template <typename T> class CoherenceSetPattern;
}
namespace HN {
...
private:
  ...
  const matrix_type localField(const int index,
                               const std::vector<neurons_type> &input);
...
public:
  ...
  const std::vector<neurons_type>
  resolvePattern(const std::vector<neurons_type> &array) const;
  void resolvePattern(std::vector<neurons_type> &getVector,
                      std::vector<float> &getEnergy,
                      std::atomic<float> *status = nullptr);

  template <typename Iterable, typename Extractor>
  void trainNetworkWithPseudoinverse(const Iterable &patterns,
                                     const Extractor extractor,
                                     std::atomic<float> *status = nullptr);
  template <typename Iterable, typename Extractor>
  void trainNetworkWithHebb(const Iterable &patterns, const Extractor extractor,
                            std::atomic<float> *status = nullptr);
...
void HN::HopfieldNetwork<neurons_type, matrix_type>::trainNetworkWithHebb(
    const Iterable &patterns, const Extractor extractor,
  ...
  const matrix_type norm_factor =
      static_cast<matrix_type>(1.0) / static_cast<matrix_type>(numberNeurons);

  const int totalIteration = patterns.size();
  int count = 0;

  W_ij = Eigen::Matrix<matrix_type, Eigen::Dynamic, Eigen::Dynamic>::Zero(
  ...
  for (const auto &pattern_container : patterns) {
    ...
    if (status) {
      ...
      count++;
      status->store(static_cast<float>(count) / totalIteration,
...
template <typename neurons_type, typename matrix_type>
template <typename Iterable, typename Extractor>
void HN::HopfieldNetwork<neurons_type, matrix_type>::
    trainNetworkWithPseudoinverse(const Iterable &patterns,
                                  const Extractor extractor,
                                  std::atomic<float> *status) {
  ...
  Eigen::Matrix<matrix_type, Eigen::Dynamic, Eigen::Dynamic> X(numNeurons,
                                                               numPatterns);
  int p_idx = 0;
  for (const auto &pattern_container : patterns) {
  ...
  Eigen::Matrix<matrix_type, Eigen::Dynamic, Eigen::Dynamic> C =
      X.adjoint() * X;

  if (std::abs(C.determinant()) <
      std::real(std::numeric_limits<matrix_type>::epsilon())) {
    throw std::runtime_error("The patterns are linearly dependent, the "
                             "correlation matrix is not invertible.");
  }

  Eigen::Matrix<matrix_type, Eigen::Dynamic, Eigen::Dynamic> C_inv =
      C.inverse();
  ...
  for (size_t i = 0; i < numNeurons; ++i) {
    ...
    if (status) {
      ...
      status->store(static_cast<float>(i) / numNeurons,
                    std::memory_order_relaxed);
    }
  }
}
