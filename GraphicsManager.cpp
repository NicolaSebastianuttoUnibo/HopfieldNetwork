...
void GraphicsManager::initialize() {
    ...
    window_ = SDL_CreateWindow(
        "Rete di Hopfield", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED,
        1280, 720, SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE);
    if (!window_) {
        throw std::runtime_error(std::string("Errore SDL_CreateWindow: ") + SDL_GetError());
    }

    glContext_ = SDL_GL_CreateContext(window_);
    if (!glContext_) {
        throw std::runtime_error(std::string("Errore SDL_GL_CreateContext: ") + SDL_GetError());
    }
    ...
    ImGui::CreateContext();
    io_ = &ImGui::GetIO();
    io_->ConfigFlags |= ImGuiConfigFlags_NavEnableKeyboard; // Abilita controllo da tastiera
    io_->ConfigFlags |= ImGuiConfigFlags_NavEnableGamepad;  // Abilita controllo da gamepad
