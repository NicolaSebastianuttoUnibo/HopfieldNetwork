...
void DrawHopfieldGridAsTable(TantiPattern::TantiPattern& tp, std::vector<int>& data, int cols, int rows, float size, bool clickable, const char* unique_id, int index) {
    ...
    // is_hovered è la variabile che ci dice se il mouse è sopra il nostro InvisibleButton (l'intera griglia)
    ...
    for (int row = 0; row < rows; ++row) {
        for (int col = 0; col < cols; ++col) {
            ...
            if (static_cast<std::vector<int>::size_type>(index) >= data.size()) continue;
    ...
    if (clickable && is_grid_hovered) { 
        ImVec2 mouse_pos_in_canvas = ImVec2(ImGui::GetMousePos().x - canvas_p0.x, ImGui::GetMousePos().y - canvas_p0.y);
        ...
        if (hovered_col >= 0 && hovered_col < cols && hovered_row >= 0 && hovered_row < rows) {
            int hovered_index = hovered_row * cols + hovered_col;
            if (static_cast<std::vector<int>::size_type>(hovered_index) < data.size()) {
                ...
                draw_list->AddRectFilled(ImVec2(hx0, hy0), ImVec2(hx1, hy1), COLOR_HOVER);
...
int main(int,char**){
    ...
    SDL_Window* window = SDL_CreateWindow("Rete di Hopfield", SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 800,600, SDL_WINDOW_OPENGL |  SDL_WINDOW_RESIZABLE);
    ...
    allpatterns.applicapixelatura(w,h);
       ...
       ImGui::Separator();
        if (ImGui::Button("Open File Dialog")) {
            ...
            config.path = ".";
            ImGuiFileDialog::Instance()->OpenDialog("ChooseFileDlgKey", "Choose File", ".png, .jpg, .jpeg, .bmp, .gif", config);
        }
        // display
       ...
       if(ImGui::Button("Save Images")){allpatterns.creaMatrice();evolvable=true;}
...
ImGui::BeginGroup();
       if(ImGui::Button("Elimina")){allpatterns.removePattern(i);ImGui::EndGroup();ImGui::End(); 
  break;}

    ImGui::Text("Pattern pixelato originale:");
    DrawHopfieldGridAsTable(allpatterns, allpatterns.getPatterns()[i].getPatternQ(), w, h, 300.0f, false, ("grid_originale"+ std::to_string(i)).c_str(),i);
ImGui::EndGroup(); 
...
ImGui::BeginGroup();
    ...
    DrawHopfieldGridAsTable(allpatterns, allpatterns.getPatterns()[i].getPatternC(), w, h, 300.0f, true, ("grid_corrotta"+ std::to_string(i)).c_str(),i);
       if(ImGui::Button("Corrompi")){allpatterns.corrompi(i);}

    ImGui::EndGroup(); 

    ImGui::BeginGroup();
    ImGui::Text("Pattern pixelato evoluzione:");
    DrawHopfieldGridAsTable(allpatterns, allpatterns.getPatterns()[i].getPatternD(), w, h, 300.0f, false, ("grid_evoluzione"+ std::to_string(i)).c_str(),i);
       if(ImGui::Button("Evoluzione")&&evolvable){allpatterns.disicorrompi(i);}
