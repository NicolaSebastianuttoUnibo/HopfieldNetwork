...
TEST_SUITE("CoherenceSetPattern") {
 ...
 auto points = MD::getMathematicalVertex<int>();
        ...
        CHECK(csp_img.getNoisyPatternVector() != csp_img.getTrainingPatternVector());
        CHECK(csp_img.getEvolvingPatternVector() == csp_img.getNoisyPatternVector());

        RU::seedGenerator(200);
        CSP::CoherenceSetPattern<int> csp_rand(0.0f, 2, 2);
        REQUIRE(csp_rand.getTrainingPatternVector().size() == 4);
        CHECK(csp_rand.getNoisyPatternVector() == csp_rand.getTrainingPatternVector());
        CHECK(csp_rand.getEvolvingPatternVector() == csp_rand.getNoisyPatternVector());
        ...
        CHECK(csp_rand.getNoisyPatternVector()[0] == points[(0 + 1) % points.size()]);
        CHECK(csp_rand.getEvolvingPatternVector()[0] == points[(0 + 1) % points.size()]);
        CHECK(csp_rand.getNoisyPatternVector()[1] == initial_noisy[1]);
        
        csp_rand.reCorrupt(0.0f); 
        CHECK(csp_rand.getNoisyPatternVector() == csp_rand.getTrainingPatternVector());
        CHECK(csp_rand.getEvolvingPatternVector() == csp_rand.getTrainingPatternVector());
        ...
        CHECK_THROWS_AS(csp_rand.updateEvolvingState({points[0], points[1]}), std::invalid_argument);
